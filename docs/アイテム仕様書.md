...
### ItemDataTableManager取得

**Get Game Instance → Get Subsystem (Class: Item Data Table Manager)**

### アイテム一覧取得

| 関数名 | 引数 | 戻り値 | 説明 |
|--------|------|--------|------|
| **Get All Items** | なし | `Array of Item Data Row` | 全アイテムデータを取得 |
| **Get Items by Type** | `Item Type Table` | `Array of Item Data Row` | 指定タイプのアイテム配列 |
| **Get Items by Quality** | `Item Quality Table` | `Array of Item Data Row` | 指定品質のアイテム配列 |
| **Get All Item Row Names** | なし | `Array of Name` | 全アイテムの行名を取得 |

### アイテム詳細取得

| 関数名 | 引数 | 戻り値 | 説明 |
|--------|------|--------|------|
| **Get Item Data** | `String ItemId` | `Item Data Row` + `Boolean` | アイテム詳細データ |
| **Get Item Data by Row Name** | `Name RowName` | `Item Data Row` | 行名でアイテム取得 |
| **Is Valid Item** | `String ItemId` | `Boolean` | 有効なアイテムか |

### 装備関連

| 関数名 | 引数 | 戻り値 | 説明 |
|--------|------|--------|------|
| **Can Equip to Slot** | `String ItemId`, `Equipment Slot Table` | `Boolean` | 装備可能か判定 |
| **Item Blocks Shield** | `String ItemId` | `Boolean` | 盾装備を阻害するか |

### 価格取得

| 関数名 | 引数 | 戻り値 | 説明 |
|--------|------|--------|------|
| **Get Item Value** | `String ItemId` | `Integer` | 基準価値 |
| **Get Modified Item Value** | `String ItemId` | `Integer` | 品質補正済み価値 |

### 品質補正値取得（FItemDataRowから）

| 関数名 | 引数 | 戻り値 | 説明 |
|--------|------|--------|------|
| **Get Modified Attack Power** | なし | `Integer` | 品質補正済み攻撃力 |
| **Get Modified Defense** | なし | `Integer` | 品質補正済み防御力 |
| **Get Modified Durability** | なし | `Integer` | 品質補正済み耐久度 |
| **Get Modified Value** | なし | `Integer` | 品質補正済み価値 |
...
1. **全武器リスト取得**
   ```
   Get Game Instance → Get Subsystem (Item Data Table Manager) → Get Items by Type (Weapon)
   ```

2. **マスターワーク品リスト取得**
   ```
   Get Game Instance → Get Subsystem (Item Data Table Manager) → Get Items by Quality (Masterwork)
   ```

3. **アイテム詳細とクオリティ補正値表示**
   ```
   Get Game Instance → Get Subsystem (Item Data Table Manager) → Get Item Data (ItemId)
   → Get Modified Attack Power
   ```
